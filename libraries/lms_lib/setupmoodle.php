<?php
        ...
        //require_once($HULK->lms_dirroot.'/config.php');
        $CFG->dbtype    = 'mysqli';$CFG->dblibrary = 'native';$CFG->dbhost    = 'localhost';
        $CFG->dbname    = 'moodle';
        $CFG->dbuser    = 'moodleadmin';
        $CFG->dbpass    = 'myadmin';
        $CFG->prefix    = 'mdl_';
        $CFG->dboptions = array (  'dbpersist' => 0,  'dbport' => '',  'dbsocket' => '',);
        $CFG->wwwroot   = 'https://'.$_SERVER['SERVER_NAME'].'/crm_new';
        $CFG->dataroot  = '/var/www/moodledata';
        $CFG->admin     = 'admin';
        $CFG->directorypermissions = 0777;
        $CFG->passwordsaltmain = 'laprofundaproydesasalada';
      //  $CFG->defaultblocks_override = 'course_proydesa,activity_modules:attendance,news_items,calendar_upcoming,recent_activity';
        $CFG->defaultblocks = 'course_proydesa,activity_modules:attendance,news_items,calendar_upcoming,recent_activity';
...
if (defined('BEHAT_SITE_RUNNING')) {
    ...
    require_once($CFG->dirroot . '/../lib/behat/lib.php');
...
if (!isset($CFG->wwwroot) or $CFG->wwwroot === 'https://example.com/moodle') {
    if (isset($_SERVER['REMOTE_ADDR'])) {
        header($_SERVER['SERVER_PROTOCOL'] . ' 503 Service Unavailable');
    }
 ...
 *
 * $H_USER is stored in the session.
 *
 * Items found in the user record:
 *  - $H_USER->email - The user's email address.
 *  - $H_USER->id - The unique integer identified of this user in the 'user' table.
 *  - $H_USER->email - The user's email address.
 *  - $H_USER->firstname - The user's first name.
 *  - $H_USER->lastname - The user's last name.
 *  - $H_USER->username - The user's login username.
 *  - $H_USER->secret - The user's ?.
 *  - $H_USER->lang - The user's language choice.
 *
 * @global object $H_USER
 * @name $H_USER
 */
global $H_USER;
...
if (defined('BEHAT_SITE_RUNNING') && !defined('BEHAT_TEST')) {
    require_once($CFG->dirroot . '/behat/lib.php');
...
// Start session and prepare global $SESSION, $H_USER.
...
if (!empty($CFG->debugvalidators) and !empty($CFG->guestloginbutton)) {
    if ($CFG->theme == 'standard') {    // Temporary measure to help with XHTML validation
        if (isset($_SERVER['HTTP_USER_AGENT']) and empty($H_USER->id)) {      // Allow W3CValidator in as user called w3cvalidator (or guest)
...
if ($H_USER && function_exists('apache_note')
    && !empty($CFG->apacheloguser) && isset($H_USER->username)) {
    $apachelog_userid = $H_USER->id;
    $apachelog_username = clean_filename($H_USER->username);
    $apachelog_name = '';
    if (isset($H_USER->firstname)) {
        // We can assume both will be set
        // - even if to empty.
        $apachelog_name = clean_filename($H_USER->firstname . " " .
                                         $H_USER->lastname);
    }
